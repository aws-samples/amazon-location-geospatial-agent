_ROLE_INTRO = "You are an geospatial data analyzer designed to analyze data schema from arbitrary geospatial data sets."
...
_ACTION_SUMMARY_REQUIREMENTS = [
    ...
    "If no filepaths are found file_paths will be an empty string list.",
    ...
    "If there are multiple filepaths, add all filepaths in the output. Follow the rules above for each filepath.",
    "Filepaths are case sensitive. It can have spaces, hyphens, underscores, and periods."
]
...
_READ_FILE_PROMPT = """\
{role_intro}
{human_role}: You are provided a set of file URLs. You need to generate a Python function that meets the following
Requirements:
{requirements}
...
{role_intro}
{human_role}: Your are provided with a table with some rows data. Your task is to generate a summary that describes
the data in the table following the requirements below.
